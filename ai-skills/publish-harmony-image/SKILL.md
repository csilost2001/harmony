...
Docker / Podman build / push は **host WSL2 bash または Linux host** で実行する。Dev Container 内には container engine access を前提にしない。
...
- smoke 結果
- push 結果
- 利用者向け `compose.yml` / `.env.example` の変更有無
...
- `scripts/smoke-harmony-compose.sh`
- `compose.yml`
- `.env.example`
- `docs/setup/distribution-roadmap.md`
- `docs/setup/compose-runtime.md`
