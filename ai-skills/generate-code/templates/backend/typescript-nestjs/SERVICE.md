...
| `steps` (kind=transactionScope) | `prisma.$transaction(async (tx) => { ... })` または `dataSource.transaction('READ COMMITTED', async manager => { ... })` で `steps[]` 内の全 step を 1 TX で wrap。詳細は本ファイル末尾の「transactionScope mapping」セクション (#875 / #1221 で旧 txBoundary 廃止し transactionScope に一本化) |
...
## transactionScope mapping (#875 / #1221 — `step.kind = "transactionScope"`)

Harmony spec の TX 表現は `step.kind = "transactionScope"` に一本化されている (#1221 で旧 `txBoundary` の平坦モデルは廃止)。実装では **必ず `$transaction()` で wrap** すること (#875: 未対応で生成すると silent semantic bug になる)。
...
**transactionScope.steps[] 内の全 step を $transaction 内に展開すること**。1 つでも漏らすと TX 範囲が崩れる silent bug の元になる。
...
export class PostsService {
  ...
  async create(userId: number, dto: CreatePostDto) {
    ...
    // step-tx (kind=transactionScope) の steps[] 全部: 1 TX で wrap
    return await this.prisma.$transaction(async (tx) => {
      ...
      // step-05: loop INSERT post_tags + tag 解決 (tx 経由)
      if (dto.tags) {
      ...
      // step-06: UPDATE tags.usage_count (tx 経由、TX commit と一体)
      // ...
...
5. **isolation level**: `prisma.$transaction(fn, { isolationLevel: 'ReadCommitted' })` で指定可 (PostgreSQL / MySQL / SQL Server のみ)。spec の `transactionScope.isolationLevel` が指定されている場合はそれを優先、未指定なら default。⚠️ **SQLite (`database.type=sqlite`) では `isolationLevel` オプション指定不可** — Prisma が `PrismaClientUnknownRequestError` を throw する。SQLite ターゲットでは `$transaction(fn)` をオプションなしで呼ぶこと。ROM 別の default は engine 依存 (PostgreSQL: ReadCommitted、SQLite: serializable に近い WAL モード、MySQL InnoDB: RepeatableRead)
...
このテストが **fail する** = `/generate-code` が `transactionScope` を honor せず TX wrap を生成していない (#875 の症状、#1221 で旧 txBoundary 廃止)。skill 利用者は生成コードに対してこの種の test を最低 1 件は必ず実行することを推奨。
