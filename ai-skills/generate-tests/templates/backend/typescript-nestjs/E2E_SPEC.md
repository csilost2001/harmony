 ...
 *   → TX rollback テスト: 故意 UNIQUE 違反 → scope 内全 rollback 確認 (#1221 で旧 txBoundary 廃止)
 *
...
| `step.kind=transactionScope` | 故意失敗 TX | `expect(res.status).toBe(500 or 409)` + Prisma 残存確認 (#1221 で旧 txBoundary 廃止し本パターンに一本化) |
 ...
 *
 * 【spec ↔ impl 乖離検出器】
 *   このテストが fail する場合、transactionScope step (${STEP_TX_SCOPE_ID}) が
 ...
 *   transactionScope の steps[] 配下 (#1221 で旧 txBoundary 廃止)
 *
