...
- **`/document-ui [<screen-key>]`** — UI 各画面の操作リファレンス (`docs/user-guide/ui-reference/`) を Playwright MCP の screenshot + a11y snapshot で自動生成 / 更新。引数省略で全 27 画面、`<screen-key>` 指定で 1 画面のみ。dev server 起動が前提 (`ai-skills/document-ui/SKILL.md`、#1401)
