 ...
 *
 * isAutoGeneratedId は frontend/src/utils/screenItemNaming.ts (#333) と同一ロジック。
 * backend は独立パッケージのため複製して保持する。
 */
...
// ── 自動生成 ID 判定 (frontend/src/utils/screenItemNaming.ts と同一) ─────────
...
// frontend/src/utils/screenItemExtractor.ts の CUSTOM_TYPE_TO_TAG と同一マッピング。
