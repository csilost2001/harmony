...
2. **DnD のテストが欠落**: 仕様書には「左パレットからドロップ」が要件として記載されているが、Puck 内部の DnD ライブラリのセレクタが特殊なため初期 E2E では省略していた → **[x] 解消 (#1420 / #1421)**: `dragPaletteItemToCanvas` helper を `frontend/e2e/helpers/puck.ts` に実装し、`puck-external-components.spec.ts` で palette → iframe canvas への実 drag/drop 配置を検証 (`@regression`)。成立条件は 編集モード + `scrollIntoViewIfNeeded` + `page.mouse` (CDP input、iframe 境界越え) + 初期 jiggle >5px + 多段 move。旧 `dragPrimitiveTo` / `dragPrimitiveByKeyboard` (非動作) は削除済
