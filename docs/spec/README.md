...
| **[generic-definition-layer.md](generic-definition-layer.md)** | **汎用設計定義レイヤー (RFC v0.1)** — 既存 entity 構造化拡張 + Generic Definition Catalog (8 kind: data-contract / domain-type / exception-type / application-rule / ui-behavior / runtime-policy / component-definition / ui-fragment) + Importer 責務分担。Markdown 設計書取り込みで欠落する例外/共通定義/設定/UI 振る舞いを言語非依存で受ける | #1060 |
| **[import-procedure.md](import-procedure.md)** | **Markdown → Harmony JSON 共通 Importer 手順書 (RFC v0.1)** — 10 ステップ標準フロー / project profile 適用ポイント / archetype 分類 / generic-definition 退避 / audit / human review gate / 再実行決定性 | #1060 |
