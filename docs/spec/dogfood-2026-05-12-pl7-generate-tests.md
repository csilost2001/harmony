...
### 3-2. 修正対象外 (本 dogfood の scope outside、別 follow-up)

#### 候補 D-X: Spring Security 認証済みセッションでの MockMvc テストヘルパ

`@WithMockUser` / `SecurityMockMvcRequestPostProcessors.user()` を使った認証済みセッション設定の
具体的なテンプレート例を SKILL.md に追加する候補。

**注**: ISSUE #1035 sub-section D は logout stub 解消 + Security 連携 (本体実装) であり、
test テンプレ拡充とは別領域。混同しないこと。これは本 PR 完了後の **新規 follow-up ISSUE 起票** で受ける。
...
**申し送り** (本 PR 完了後の follow-up):
- Java 21 + Maven + Spring Boot アプリ構成 (e.g. docker maven:3.9-eclipse-temurin-21 image) の準備 + 実機 maven test 実行 → **新規 follow-up ISSUE 起票で trace**
- NestJS/Next.js 系サンプル (examples/english-learning-tailwind 等) での vitest/Playwright 実機 dogfood → **同 follow-up ISSUE または別 ISSUE で trace**

## 5. follow-up (本 dogfood で trace を取る分)

1. **実機 maven test 環境の準備 + smoke 実行** (Java 21 + Maven 環境)
   → 本 PR merge 後に新規 follow-up ISSUE 起票で trace
2. **NestJS/Next.js 系での vitest 実機 dogfood**
   ...
   → 上記 follow-up ISSUE に sub-section として含めるか、別 ISSUE で起票
