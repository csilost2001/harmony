...
`designFileRef` と `puckDataRef` は **どちらか一方のみ** を持つ。どちらを持つかは `editorKind` が宣言する。**この排他は #1185 提案 D (実装 PR #1194) で allOf+if/then として schema 強制済**。`then` 側に `required` を置かず `not + required` のみで「不一致 ref の同時指定禁止」を表現することで、draft-state policy (editorKind だけ書いて ref 未設定) を許容する (Codex 改善 sketch 採用)。
