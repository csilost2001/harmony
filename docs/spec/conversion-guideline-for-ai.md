...
**重要**: 本 cheatsheet は schema を機械抽出した snapshot (`.tmp/extract-step-required.mjs` + `.tmp/extract-nested-required.mjs` 相当)。schema 更新時は再生成すべき。spec 編集者の記憶に頼って書き換えない。不確実な場合は必ず `schemas/v3/process-flow.v3.schema.json` の該当 `$defs/<XxxStep>` / `$defs/<NestedDef>` を Read で確認。
