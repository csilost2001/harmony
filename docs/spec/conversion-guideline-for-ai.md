...
**Step kind ごとの必須 field cheatsheet** (`description` は全 kind で必須なので省略):

| step kind | 必須 field (description 以外) | 用途 |
|---|---|---|
...
| `branch` | `branches[]` (各 branch に condition / steps[]) | 条件分岐 |
| `displayUpdate` | `target` (string) | 画面表示更新 |
| `screenTransition` | `targetScreenId` | 画面遷移 |
| `loop` | `loopKind`, その他 loop variant 別 | 繰り返し |
| `loopBreak` / `loopContinue` | (なし) | loop 制御 |
| `jump` | `jumpTo` | ステップジャンプ |
| `log` | `level`, `message` | ログ出力 |
| `audit` | `action` | 監査ログ |
| `externalSystem` | `systemRef` | 外部システム呼び出し |
| `transactionScope` | (variant 別) | TX 境界明示 |
| `eventPublish` / `eventSubscribe` | `topic` 等 | event pub/sub |
| `workflow` | variant 別 | 承認等パターン |
| `closing` / `cdc` / `aiCall` / `aiAgent` / `extension` | variant 別、schema 参照 | 各種特化 |

不確実な場合は `schemas/v3/process-flow.v3.schema.json` の該当 `$defs/<XxxStep>` を必ず Read で確認。
