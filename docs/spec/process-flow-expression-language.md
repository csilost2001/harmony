...
| 記法 | 解決先 | 例 |
|---|---|---|
| `@conv.<category>.<key>` | `Conventions` catalog (`conventions.v3.schema.json`)。built-in (msg/regex/limit/scope/currency/tax/auth/role/permission/db/numbering/tx/externalOutcomeDefaults) と `extensionCategories.<categoryName>` を **同一名前空間** で解決する (#1221)。`@conv.extensionCategories.<cat>.<key>` 形式は禁止 — 拡張カテゴリも `@conv.<categoryName>.<key>` で参照 | `@conv.regex.email` / `@conv.limit.lowStockThreshold` / `@conv.role.manager` / 拡張: `@conv.wordProgress.masteryThreshold` |
...
### 6.5a `ValidationStep.fieldErrorsVar` (#1221 で明示必須化)

`ValidationStep.rules[]` の評価結果を格納する変数名を明示するフィールド。**schema-level required** (旧 `"fieldErrors"` 既定 + `@fieldErrors` 暗黙束縛は #1221 で廃止)。慣習的に `"fieldErrors"` を推奨するが、複数 validation step が並ぶ場合は step ごとに別名を付ける。

`inlineBranch.ngBodyExpression` や後続 step は **宣言された変数名そのもの** で参照する: `fieldErrorsVar: "fieldErrors"` 宣言時は `@fieldErrors`、`fieldErrorsVar: "myErrors"` 宣言時は `@myErrors`。

型は `Record<fieldName, errorMessage>`。step-local binding (`ambientVariables` への明示宣言は不要)。
