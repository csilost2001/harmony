 ...
 * D-7: database.type="sqlite" → workers: 1, fullyParallel: false (SQLite 共有 DB)
 */
...
test.describe('ダッシュボード → 学習セッション → セッション結果 E2E', () => {
  ...
  test('ストーリー選択 → 学習セッション開始 → 会話プレイ画面へ遷移 (step 3)', async () => {
    ...
    const sessionResponse = page.waitForResponse(
      (response) =>
        response.url().includes('/api/el/sessions') &&
        response.request().method() === 'POST' &&
        response.status() === 201,
    );

    // 「学習開始」ボタンクリック (data-testid="startLearningButton")
    await page.getByTestId('startLearningButton').click();
  ...
  test('セッション結果画面が表示される (step 4)', async () => {
    // seededData.sessionId を使ってセッション結果画面へ直接アクセス
    const sessionId = seededData.sessionId ?? 1;
  ...
  test('完全シナリオ: ダッシュボード → 学習開始 → セッション結果', async () => {
    ...
    // Step 2: 「学習開始」ボタンでセッション開始 API を呼び出し
    const sessionResp = page.waitForResponse(
      (r) =>
        r.url().includes('/api/el/sessions') &&
